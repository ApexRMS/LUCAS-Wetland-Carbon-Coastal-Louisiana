...
# General (non-scenario-specific) setup - computed once, outside the
...
# Per-GWP-variant scenario lookup and mapping ----------------------------
...
for (activeGWP in names(gwpVariants)) {
  ...
  if(!dir.exists(pathOutMaps)){
    dir.create(pathOutMaps, recursive = TRUE)
  }

  myScenario <- getScenarioExact(myProject, vTag("Basin Baseline", gwpVariant, style="suffix"))
