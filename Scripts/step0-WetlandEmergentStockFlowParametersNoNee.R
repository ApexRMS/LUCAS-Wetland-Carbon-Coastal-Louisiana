...
abovegroundPlot <- read_csv(paste0(cleanDataDir, 'LUCAS_AbovegroundPlot2012-2013_calcs_ER73_Stagg_2019-08.csv'))
belowgroundPlot <- read_csv(paste0(cleanDataDir, 'LUCAS_BelowgroundPlot2012-2013_calcs_ER73_Stagg_2019-08.csv'))
...
if(download_data) download.file(litterBagUrl, tempFilepath)
litterbagPlot <- read_csv(tempFilepath)

litterbagAbovePlot <- read_csv(paste0(cleanDataDir,"FSP_Stagg_Decomp_AbovegroundLitterDecomposition.csv"))
litterbagForestPlot <- read_csv(paste0(cleanDataDir,"FSP_Stagg_GCC_Decomp_Decomposition.csv"))
...
if(download_data) download.file(soilCoreShortUrl, tempFilepath, cacheOK=F)
...
if(download_data) download.file(soilCoreLongUrl, tempFilepath, cacheOK=F)
...
burialSiteRaw <- read_csv(paste0(cleanDataDir, 'Baustian_Long_term_Accretion_Table2_Updated_03-09-2023.csv'))


# Data provided directly from Camille Stagg, USGS
# *****BR****** Need to check if there is a data release for these data
soilShoreWaterRaw <- read_csv(paste0(cleanDataDir, "Dataset_02_macroclimate_soil_data_2_22_2016.csv"))
vegShoreWaterRaw <- read_csv(paste0(cleanDataDir, "Dataset_01_macroclimate_vegetation_data_all_2_24_2016.csv"))

# Wetland crosswalk
LUCASWetlandSubclassRaw <- read_csv(paste0(cleanDataDir, "LUCAS_Wetland_Subclass.csv"))

LUCASWetlandSubclassRaw <- LUCASWetlandSubclassRaw %>%
  rename(`Wetland Subclass LUCAS Older` = `Wetland Subclass LUCAS`,
         ExternalVariableValueOlder = ExternalVariableValue) %>%
  mutate(`Wetland Subclass LUCAS` = case_when((`Habitat Type 2014` %in% c("Saline",
                                                                          "Brackish",
                                                                          "Intermediate")) ~ "Estuarine Herbaceous",
...
biomassToCarbonRaw <- read_csv(paste0(cleanDataDir, "biomass-to-carbon-site.csv"))

# Literature values for lateral flux
lateralFluxRaw <- read_csv(paste0(cleanDataDir, "wetlandHerbaceousSummary_StockBasedEquilibrium_2023-01-16.csv"))
...
lateralFluxRaw <- lateralFluxRaw %>%
  filter(Source != "Mean") %>%
  mutate(`Wetland Subclass LUCAS` = case_when((Salinity %in% c("Salt",
                                                               "Brackish")) ~ "Estuarine Herbaceous",
                                              (Salinity %in% c("Fresh")) ~ "Palustrine Herbaceous"))

lateralFluxType <- lateralFluxRaw %>%
  group_by(`Wetland Subclass LUCAS`) %>%
  summarize(TotalLatTrans_Amount = round(mean(Flux.g.C.per.m2.per.y, na.rm = T),1))


## Litter bag data
litterbagPlot <- litterbagPlot %>%
  select(where(function(x) all(!is.na(x)))) %>%
  filter(`Sampling Event` == "T4",
         `Mass Remaining (%)` != ".") %>%
  mutate(`BGVF.annual.percent.loss` = 100 - as.numeric(`Mass Remaining (%)`),
         Site.ID = case_when(`Site ID` == "BA0104" ~ "BA-01-04",
...
litterbagAbovePlot <- litterbagAbovePlot %>%
  ...
  filter(`Sampling Event` == "T4",
         `Mass Remaining (%)` != ".") %>%
  mutate(`AGVF.annual.percent.loss` = 100 - as.numeric(`Mass Remaining (%)`),
         Site.ID = case_when(`Site ID` == "BA0104" ~ "BA-01-04",
                             nchar(`Site ID`) == 3 ~ paste0("CRMS0", `Site ID`),
                             nchar(`Site ID`) == 4 ~ paste0("CRMS", `Site ID`),
                             nchar(`Site ID`) > 4 ~ `Site ID`)) %>%
...
soilIncrementDict <- data.frame(Core_Increment = c(1, 2, 3, 4, 5),
...
soilCorePlotShort <- soilCorePlotShort %>% 
  ...
  mutate(`Core Increment (cm)` = Core_Increment_New,
         `Organic matter (percent)` = Organic_Matter,
         `Bulk Density (g/cm^3)` = Bulk_Density) %>%
  select(Site, `Core Increment (cm)`, `Organic matter (percent)`, `Bulk Density (g/cm^3)`)
...
soilCorePlot <- soilCorePlot %>%
  mutate(Site.ID = case_when(nchar(Site) == 3 ~ paste0("CRMS0", Site),
...
LUCASWetlandSubclass <- LUCASWetlandSubclassRaw %>% select(`Site.ID`, `Wetland Subclass LUCAS`)
abovegroundPlot <- abovegroundPlot %>% left_join(LUCASWetlandSubclass, by = 'Site.ID')
belowgroundPlot <- belowgroundPlot %>% left_join(LUCASWetlandSubclass, by = 'Site.ID')
litterbagPlot <- litterbagPlot %>% left_join(LUCASWetlandSubclass, by = 'Site.ID')
soilCorePlot <- soilCorePlot %>% left_join(LUCASWetlandSubclass, by = 'Site.ID')
burialSiteRaw <- burialSiteRaw %>% left_join(LUCASWetlandSubclass, by = 'Site.ID')
litterbagAbovePlot <- litterbagAbovePlot %>% left_join(LUCASWetlandSubclass, by = 'Site.ID')
...
abovegroundStocksSite <- abovegroundPlot %>%
  ...
  summarize(foliageStockB = mean(`Live Biomass.g`/Plot.Area.m2, na.rm = TRUE), 
            AGVFStockB = mean(`Dead Biomass.g`/Plot.Area.m2, na.rm = TRUE)) %>%
  left_join(biomassToCarbonAbove, by = "Site.ID") %>%
  mutate(foliageStock = foliageStockB*(btcLive/100),
...
soilCorePlot$`Organic matter (percent)`[soilCorePlot$`Organic matter (percent)` == "."] <- NA
soilCorePlot$`Organic matter (percent)` <- as.numeric(soilCorePlot$`Organic matter (percent)`)
# total carbon density = bulk density * organic matter % * conversion factor OM to TC
soilCorePlot <- soilCorePlot %>%
  mutate(`Total Carbon (percent)` = `Organic matter (percent)` * conversionOMtoTC,
...
soilCorePlot <- soilCorePlot %>%
  separate(col = `Core Increment (cm)`, into = c("min.depth.cm", "max.depth.cm"), sep="-") %>%
  mutate(min.depth.cm = as.numeric(min.depth.cm),
         max.depth.cm = as.numeric(max.depth.cm))

# BGS C in 0-30cm
BGSStockSiteTop <- soilCorePlot %>%
  filter(max.depth.cm < BGVFdepth) %>%
  mutate(depth.interval = max.depth.cm - min.depth.cm,
         weighted.TC.g.m.2.per.cm = (depth.interval)*TC.g.m.2.per.cm) %>%
  group_by(Site.ID) %>%
  drop_na() %>%
  # Weighted mean calculation
  summarize(MeanCDensityBGSTopwithRootBGVF = sum(weighted.TC.g.m.2.per.cm, na.rm=TRUE) / sum(depth.interval, na.rm=TRUE)) %>%
  ...
  mutate(BGSStockTop = TCTop - BGVFStock - RootStock, 
...
BGSStockSiteBottom <- soilCorePlot %>%
  ...
  summarize(MeanCDensityBGSBottom = sum(weighted.TC.g.m.2.per.cm, na.rm=TRUE) / sum(depth.interval, na.rm=TRUE), 
...
BGSStockSite <- BGSStockSiteBottom %>%
  ...
  mutate(BGSStock = BGSStockTop + BGSStockBottom, 
         'Mean BGSCDensity TC.g.m.2.per.cm' = (BGSStockTop + BGSStockBottom) / BGSdepth) %>%
  select(Site.ID, `Wetland Subclass LUCAS`, BGSStock, `Mean BGSCDensity TC.g.m.2.per.cm`, MeanCDensityBGSBottom, MeanCDensityBGSTop)
...
foliageNPPEvent <- abovegroundPlot %>%
  ...
  summarize(FoliageNPPEvent = mean(Interval.Production.g/Plot.Area.m2, na.rm = TRUE)) 
...
rootNPPEvent <- belowgroundPlot %>%
  ...
  summarise(RootNPPEvent = mean(Interval.Production.g/Core.Area.m2, na.rm = TRUE)) %>%
  group_by(Event, Depth,`Wetland Subclass LUCAS`) %>%
  mutate(MeanRootNPPEvent = mean(RootNPPEvent, na.rm=T), 
...
litterbagSite <- litterbagPlot %>%
  group_by(Site.ID, `Wetland Subclass LUCAS`) %>%
  summarize(BGVFPercentLoss = mean(BGVF.annual.percent.loss, na.rm = TRUE) / 100) #AGVFPercentLoss = mean(AGVF.annual.percent.loss, na.rm = TRUE) / 100,

litterbagAboveSite <- litterbagAbovePlot %>%
  group_by(Site.ID, `Wetland Subclass LUCAS`) %>%
  summarize(AGVFPercentLoss = mean(AGVF.annual.percent.loss, na.rm = TRUE) / 100) #AGVFPercentLoss = mean(AGVF.annual.percent.loss, na.rm = TRUE) / 100,


# Calculate burial rates
burialSiteDensity <- burialSiteRaw %>%
  mutate('Mean Uncorr-210Pb Accr. (cm  yr-1)' = mean(`Uncorr-210Pb Accr. (cm  yr-1)`, na.rm=T), 
         'Uncorr-210Pb Accr. (cm  yr-1)' = ifelse(is.na(`Uncorr-210Pb Accr. (cm  yr-1)`), `Mean Uncorr-210Pb Accr. (cm  yr-1)`, `Uncorr-210Pb Accr. (cm  yr-1)`)) %>%
  mutate(densitySampleDepth = (2 * ceiling(`Uncorr-210Pb Accr. (cm  yr-1)`/2))) %>%
  full_join(soilCorePlot, by=c('Site.ID', 'Wetland Subclass LUCAS'), multiple="all") %>%
  ...
  mutate(depth.interval = max.depth.cm - min.depth.cm, 
         weighted.TC.g.m.2.per.cm = (depth.interval)*TC.g.m.2.per.cm) %>%
  summarize(`Accretion Interval CDensity TC.g.m.2.per.cm` = sum(weighted.TC.g.m.2.per.cm, na.rm=TRUE) / sum(depth.interval, na.rm=TRUE))

burialSiteDensity %>% 
  full_join(BGSStockSite, by="Site.ID") %>% 
  mutate(`BGSCDensity Mean TC.g.m.2.per.cm` = `Mean BGSCDensity TC.g.m.2.per.cm`,
...
burialSite <- burialSiteRaw %>%
  ...
  mutate('Mean Uncorr-210Pb Accr. (cm  yr-1)' = mean(`Uncorr-210Pb Accr. (cm  yr-1)`, na.rm=T), 
         'Uncorr-210Pb Accr. (cm  yr-1)' = ifelse(is.na(`Uncorr-210Pb Accr. (cm  yr-1)`), `Mean Uncorr-210Pb Accr. (cm  yr-1)`, `Uncorr-210Pb Accr. (cm  yr-1)`)) %>%
  mutate('Burial210Pb TC.g.m.2' = `Accretion Interval CDensity TC.g.m.2.per.cm` * `Uncorr-210Pb Accr. (cm  yr-1)`) %>%
...
#  full_join(neeSite, by=c('Site.ID', 'Wetland Subclass LUCAS')) %>%
  ...
  mutate('Wetland Subclass LUCAS' = recode(`Wetland Subclass LUCAS`, 
...
funFluxCalc <- function(SummaryTab) {
  ...
  SummaryTab <- SummaryTab %>%
    mutate('TotalNPP_Amount' = FoliageNPP + RootNPP,
           ...
           'FoliageMortality_Amount' = FoliageMortality_PropMaxStock * FoliageMaxStock,
           ...
           'AGVFStabilization_PropMaxStock' = AGVFStabilization_Amount / AGVFMaxStock,
           ...
           'BGVFStabilization_Amount' = max(c(BGVFStabilization_PropOut * BGVFOut_Amount,
    ...
    mutate('BGSMaxStock' = BGSStock + BGSIn_Amount,
           ...
           'TotalStabilization_Amount' = AGVFStabilization_Amount + BGVFStabilization_Amount + BGSStabilization_Amount,
           ...
           'TotalEmissionLatTrans_Amount' = AGVFEmissionLatTrans_Amount + BGVFEmissionLatTrans_Amount + BGSEmissionLatTrans_Amount,
           ...
           # 'TotalLatTrans_PropEmissionLatTrans' = TotalLatTrans_Amount / TotalEmissionLatTrans_Amount,
           'AGVFLatTrans_Amount' = TotalLatTrans_PropEmissionLatTrans * AGVFEmissionLatTrans_Amount,
           'BGVFLatTrans_Amount' = TotalLatTrans_PropEmissionLatTrans * BGVFEmissionLatTrans_Amount,
           'BGSLatTrans_Amount' = TotalLatTrans_PropEmissionLatTrans * BGSEmissionLatTrans_Amount,
           ...
           'outAGVFCheck' = FoliageMortality_Amount-(AGVFMaxStock*(AGVFEmission_PropMaxStock + AGVFLatTrans_PropMaxStock + AGVFStabilization_PropMaxStock)),
           'outBGVFCheck' = RootMortality_Amount - (BGVFMaxStock*(BGVFEmission_PropMaxStock + BGVFLatTrans_PropMaxStock + BGVFStabilization_PropMaxStock)),
           'outBGSCheck' = BGSIn_Amount- (BGSMaxStock*(BGSEmission_PropMaxStock + BGSLatTrans_PropMaxStock + BGSStabilization_PropMaxStock)),
           # 'TotalLatTrans_Amount' = AGVFLatTrans_Amount + BGVFLatTrans_Amount + BGSLatTrans_Amount, 
           'massBalanceCheck' = TotalNPP_Amount - (AGVFOut_Amount - AGVFStabilization_Amount + BGVFOut_Amount - BGVFStabilization_Amount + BGSOut_Amount),
           ...
           'checkpropAGVF' = AGVFOut_Amount / (TotalNPP_Amount + BGVFStabilization_Amount) - AGVFOut_PropOut)
...
siteSummaryLatFlux <- siteSummary0 %>%
  select(Site.ID,`Wetland Subclass LUCAS`, FoliageStock, RootStock, AGVFStock, BGVFStock, BGSStock,
         FoliageNPP, RootNPP, BGVFPercentLoss, `Burial210Pb TC.g.m.2`,MethaneFlux,ExternalVariableValue,
         AGVFPercentLoss)
...
siteSummaryLatFlux %>%
  mutate_if(is.numeric, round, digits=sig_figs) %>%
  write_csv(paste0(outDir, "siteSummaryLatFlux_StockBasedEquilibrium_", dateStamp, ".csv"))
...
CH4ExternalDistnLatFlux <- siteSummaryLatFlux %>%
  select(Site.ID,
         ...
         'ExternalVariableID'=externalVariableLatSite, 
         'Value'=MethaneFlux) %>%
  mutate(DistributionTypeID = paste(StateClassID, "Methane Emissions"),
         ExternalVariableTypeID = paste("Site ID", StateClassID),
         ExternalVariableMin = ExternalVariableID, 
         ExternalVariableMax = ExternalVariableID,
         ValueDistributionRelativeFrequency = 1) %>%
...
subclassSummary <- ungroup(siteSummary0) %>%
  select(`Wetland Subclass LUCAS`, FoliageStock, RootStock, AGVFStock, BGVFStock, BGSStock,
         FoliageNPP, RootNPP, BGVFPercentLoss, `Burial210Pb TC.g.m.2`,TotalLatTrans_Amount, MethaneFlux,
         AGVFPercentLoss) %>%
  group_by(`Wetland Subclass LUCAS`) %>%
...
subclassSummary %>%
  mutate_if(is.numeric, round, digits=sig_figs) %>%
  write_csv(paste0(outDir, "subclassSummary_StockBasedEquilibrium_", dateStamp, ".csv"))
...
initialCarbonMean <- subclassSummary %>%
  select('StateClassID'=`Wetland Subclass LUCAS`, 
         'Carbon Initial Conditions: Foliage'=FoliageStock, 
         'Carbon Initial Conditions: Fine Roots'=RootStock, 
         'Carbon Initial Conditions: Aboveground Very Fast'=AGVFStock, 
         'Carbon Initial Conditions: Belowground Very Fast'=BGVFStock, 
...
NPPMean <- subclassSummary %>%
  select('StateClassID'=`Wetland Subclass LUCAS`, 'Net Growth'=TotalNPP_Amount) %>%  
  gather('StateAttributeTypeID', 'Value', 2)

MethaneMean <- subclassSummary %>%
  select('StateClassID'=`Wetland Subclass LUCAS`, 'Methane Emissions'=MethaneFlux) %>%  
  gather('StateAttributeTypeID', 'Value', 2)
