...
# NOTE: this script is sourced once (not per-GWP-variant) from RunAllScripts.R,
# so it loops internally over both GWP variants (see gwpConfig.R) rather than
# relying on an externally-set `activeGWP`.
...
# General (non-scenario-specific) setup: datasheets and lookups used by all
# GWP variants - computed once, outside the per-variant loop below.
...
# Per-GWP-variant scenario lookups and plotting --------------------------
# Each variant gets its own output subfolder (OutputFigures/<GWP100|GWP20>/...)
# so the two variants' PNGs never overwrite each other.
...
for (activeGWP in names(gwpVariants)) {
  ...
  # Land cover change chart for the Baseline scenario.
  # scenLabel keeps a short, readable label for output filenames;
  # scen holds the full GWP-tagged name used for the exact scenario lookup.
  scenLabel <- c("Baseline")
  scen <- c(vTag("Basin Baseline", gwpVariant, style="suffix"))

  for (s in 1:length(scen)){

    myScenarioS <- getScenarioExact(myProject, scen[s])
    ...
    ggsave(paste0(pathOutLandCover,"/LandCoverChangeWater_",gsub(" ","",scenLabel[s]),".png"), p3, width = 7.5, height = 4, dpi = 600)
...
} # end for (activeGWP in names(gwpVariants))
