...
# Loop through both GWP variants (Revised GWP-100 and Revised GWP-20).
# Everything below that looks up a scenario by name, or writes an output
# file, must run once per variant so the two variants' scenarios are found
# correctly and their outputs land in separate subfolders.

for (activeGWP in names(gwpVariants)){

  gwpVariant <- gwpVariants[[activeGWP]]

  pathOut <- paste0(rootPath,"Models/",modelName,"/Output Data Release/", gwpVariant$label, "/")
  ...
  if(!dir.exists(pathOutTabular)){
    dir.create(pathOutTabular, recursive = TRUE)
  }
  ...
  for (i in 1:length(pathOutSpatialSub)){
    if(!dir.exists(pathOutSpatialSub[i])){
      dir.create(pathOutSpatialSub[i], recursive = TRUE)
    }
  }
  ...
  for (i in 1:length(scenariosBasin)){
    ...
    myScenario <- getScenarioExact(myProject, vTag(paste0("Basin ",scenariosBasin[i]), gwpVariant, style="suffix"))
  ...
  scenID <- scenarioId(getScenarioExact(myProject, vTag(paste0("Basin ",scenariosBasin[1]), gwpVariant, style="suffix")))
  ...
  for (i in 1:length(scenariosSingleCell)){
    ...
    myScenario <- getScenarioExact(myProject, vTag(scenariosSingleCell[i], gwpVariant, style="bracket"))
