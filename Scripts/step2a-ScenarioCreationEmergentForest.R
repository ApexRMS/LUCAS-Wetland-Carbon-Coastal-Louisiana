...
# Add Flow Type
flowTypes <- read_csv(paste0(
  rootPathUpdatedTables,
  "stsimsf_FlowMultiplier/stsimsf_FlowMultiplier Wetland Emergent Mean IPCC.csv"
))
...
write.csv(
  tibble(Name = flowTypesAdd),
  paste0(outpathDatasheets, "stsim_FlowType.csv"),
  row.names = FALSE
)
...
write.csv(
  tibble(Name = flowGroupsAdd),
  paste0(outpathDatasheets, "stsim_FlowGroup.csv"),
  row.names = FALSE
)
...
write.csv(
  tibble(
    Name = "Atmosphere Temp",
    Description = "Temporary pool used for forested wetland model"
  ),
  paste0(outpathDatasheets, "stsim_StockType.csv"),
  row.names = FALSE
)
...
write.csv(
  tibble(Name = stockGroupsAdd),
  paste0(outpathDatasheets, "stsim_StockGroup.csv"),
  row.names = FALSE
)
...
write.csv(
  tibble(Name = newStateAttributes),
  paste0(outpathDatasheets, "stsim_AttributeGroup.csv"),
  row.names = FALSE
)
...
write.csv(
  tibble(
    Name = c(newStateAttributes, "Carbon Initial Conditions: Deep Soil"),
    AttributeGroupId = c(newStateAttributes, "Carbon Initial Conditions")
  ),
  paste0(outpathDatasheets, "stsim_StateAttributeType.csv"),
  row.names = FALSE
)
...
myData <- datasheet(myScenario, name = sheetName, empty = TRUE) %>%
  addRow(data.frame(TotalAmount = 1, NumCells = 1, CalcFromDist = TRUE))
...
myCSV <- read_csv(paste0(
  rootPath,
  dataPath,
  "Additional Spinups/Initial Stock - Non Spatial.csv"
))
names(myCSV) <- gsub("ID", "Id", names(myCSV))
myData <- datasheet(myScenario, name = sheetName, empty = TRUE) %>%
  addRow(myCSV) %>%
  addRow(tibble(
    StockTypeId = "Deep Soil",
...
myDataAddLatCH4 <- tibble(
  FlowTypeId = c("Emission: Atmosphere Temp -> Atmosphere: CH4"),
  Order = 6.8
)

myDataAddAtm <- tibble(
  FlowTypeId = c("Atmosphere Temp -> Atmosphere"),
  Order = 6.9
)

myDataAddLat2 <- tibble(
  FlowTypeId = c(
    "Emission Emergent: BG Slow -> Atmosphere Temp",
    "Stabilization Emergent: BG Slow -> Deep Soil",
    "Lateral Transport Emergent: BG Slow -> Aquatic",
...
write.csv(
  myDataAll,
  paste0(outpathDatasheets, "stsim_FlowOrder.csv"),
  row.names = FALSE
)
...
myScenario <- scenario(
  myProject,
  scenario = "Transition Multipliers",
  folder = "Single-Cell Sub-Scenarios"
)
...
write.csv(
  tibble(StockTypeId = "Atmosphere Temp", Location = "F6"),
  paste0(outpathDatasheets, "stsim_FlowPathwayDiagram.csv"),
  row.names = FALSE
)
...
flowTypesAddEmergent <- tibble(
  FromStateClassId = rep(
    c("Wetland: Estuarine Emergent", "Wetland: Palustrine Emergent"),
  ...
  separate_wider_delim(
    FlowTypeId,
    delim = " -> ",
    names = c("FromStockTypeId", "ToStockTypeId"),
    cols_remove = FALSE
  ) %>%
  mutate(
    FromStockTypeId = case_when(
      FromStockTypeId ==
        "Emission: AG Very Fast" ~ "DOM: Aboveground Very Fast",
...
flowTypesAddForest1 <- tibble(
  FromStateClassId = rep(
    c("Wetland: Palustrine Forested", "Wetland: Estuarine Forested"),
  ...
  mutate(
    ToStockTypeId = case_when(
      ToStockTypeId == "BG Slow" ~ "DOM: Belowground Slow"
    )
  ) %>%
  mutate(
    FromStockTypeId = case_when(
      FromStockTypeId == "Decay: AG Fast" ~ "DOM: Aboveground Fast",
...
flowTypesAddCO2 <- tibble(
  ...
  mutate(
    FromStockTypeId = case_when(
      ...
      FromStockTypeId ==
        "Emission: BG Very Fast" ~ "DOM: Belowground Very Fast",
...
myData$FlowTypeId[
  myData$FlowTypeId == "Lateral Transport: BG Slow -> Aquatic" &
    myData$FromStateClassId %in%
      c("Wetland: Estuarine Emergent", "Wetland: Palustrine Emergent")
] <- "Lateral Transport Emergent: BG Slow -> Aquatic"
myData$FlowTypeId[
  myData$FlowTypeId == "Stabilization: BG Slow -> Deep Soil" &
    myData$FromStateClassId %in%
      c("Wetland: Estuarine Emergent", "Wetland: Palustrine Emergent")
] <- "Stabilization Emergent: BG Slow -> Deep Soil"
...
write.csv(
  myDataAll,
  paste0(outpathDatasheets, "stsim_FlowPathway1.csv"),
  row.names = FALSE
)
write.csv(
  myDataState,
  paste0(outpathDatasheets, "stsim_FlowPathway2.csv"),
  row.names = FALSE
)
...
myDataAdd <- tibble(
  FlowGroupId = paste0(flowTypesAdd, " [Type]"),
  Summary = TRUE,
  Spatial = FALSE,
  AvgSpatial = FALSE
) %>%
  addRow(tibble(
    FlowGroupId = flowGroupsAdd,
    Summary = TRUE,
    Spatial = FALSE,
    AvgSpatial = TRUE
  ))
...
write.csv(
  myDataAdd,
  paste0(outpathDatasheets, "stsim_OutputFilterFlows.csv"),
  row.names = FALSE
)
...
myDataAdd <- tibble(
  StockGroupId = paste0("Atmosphere Temp", " [Type]"),
  Summary = TRUE,
  Spatial = FALSE,
  AvgSpatial = FALSE
) %>%
  addRow(tibble(
    StockGroupId = stockGroupsAdd,
    Summary = TRUE,
    Spatial = FALSE,
    AvgSpatial = TRUE
  ))
...
write.csv(
  myDataAdd,
  paste0(outpathDatasheets, "stsim_OutputFilterStocks.csv"),
  row.names = FALSE
)
...
myData$Value[
  myData$FlowTypeId %in%
    c("LULC: Emission DOM CH4", "LULC: Emission Live CH4") &
    myData$Value %in% c(33.5)
] <- GWPmethane
  ...
  filter(
    !FlowGroupId %in%
      c(
        ...
        "Q10 Fast Flows",
        "Emission: Total Rh",
        "Emission: Total Rh (CO2e)"
      )
  ) %>%
  mutate(
    FlowTypeId = case_when(
      FlowTypeId ==
        "Emission: AG Very Fast -> Atmosphere" ~ "Emission: Atmosphere Temp -> Atmosphere: CH4"
    )
  ) %>%
  mutate(
    Value = case_when(
...
write.csv(
  myDataAll,
  paste0(outpathDatasheets, "stsim_FlowTypeGroupMembership1.csv"),
  row.names = FALSE
)

# Add to model
myCSV <- read.csv(paste0(
  outpathDatasheets,
  "stsim_FlowTypeGroupMembership1.csv"
))

myData <- datasheet(
  myScenario,
  name = "stsim_FlowTypeGroupMembership",
  empty = TRUE,
  optional = TRUE
) %>%
  addRow(myCSV)
...
rm(
  ...
  myCSV,
  myDataAll,
  myDataEmissionsCH4forest,
  myDataEmissionsCO2forest,
  myFlowsForest,
  myDataLateralForest,
  myDataLateralForest1,
  myDataLateralForest2,
  myDataLateralForest3,
  myDataLateralEmergent4,
  myDataEmissionsCO2b
)
...
myFlowsRename <- myData %>%
  ...
  mutate(
    FlowGroupId = case_when(
      FlowGroupId ==
        "Net Biome Productivity (CO2e)" ~ "Annual Net Ecosystem Carbon Balance (tons CO2-eq per year)",
      FlowGroupId ==
        "Net Biome Productivity" ~ "Annual Net Ecosystem Carbon Balance (tons C per year)",
      FlowGroupId == "Lateral Flux" ~ "Annual Lateral Flux (tons C per year)",
      FlowGroupId ==
        "Lateral Flux (CO2e)" ~ "Annual Lateral Flux (tons CO2-eq per year)",
      FlowGroupId ==
        "Emission: Total (CO2e)" ~ "Annual Emissions: CO2, CO, and CH4 (tons CO2-eq per year)",
      FlowGroupId ==
        "Emission: Total" ~ "Annual Emissions: CO2, CO, and CH4 (tons C per year)",
      FlowGroupId ==
        "Net Growth: Total (CO2e)" ~ "Annual Net Growth (tons CO2-eq per year)",
      FlowGroupId == "Net Growth: Total" ~ "Annual Net Growth (tons C per year)"
    )
  )

myFlowsNetMethane <- myData %>%
  filter(FlowGroupId %in% c("Emission: Total (CO2e)", "Emission: Total")) %>%
  filter(FlowTypeId %in% grep("CH4", FlowTypeId, value = TRUE)) %>%
  mutate(
    FlowGroupId = case_when(
      FlowGroupId ==
        "Emission: Total (CO2e)" ~ "Annual Emissions: CH4 (tons CO2-eq per year)",
      FlowGroupId ==
        "Emission: Total" ~ "Annual Emissions: CH4 (tons C per year)"
    )
  )
...
myDataAdd5 <- data.frame(
  FlowTypeId = c(
    "Net Growth Wetland Emergent: Atmosphere -> Fine Roots",
...
myDataAdd7 <- myData %>%
  filter(FlowGroupId == "Emission: Total (CO2e)") %>%
  mutate(
    FlowGroupId = "Annual Emissions: CO2 and CH4 (tons CO2-eq per year)"
  ) %>%
  filter(!(FlowTypeId %in% grep("CO$", FlowTypeId, value = TRUE)))
...
myDataAdd8 <- myData %>%
  ...
  mutate(
    FlowGroupId = case_when(
      FlowTypeId == "Lateral Transport: AG Fast -> Aquatic" ~ "Q10 Slow Flows",
      FlowTypeId ==
        "Lateral Transport: AG Medium -> Aquatic" ~ "Q10 Slow Flows",
      FlowTypeId ==
        "Lateral Transport: AG Very Fast -> Aquatic" ~ "Q10 Fast Flows",
      FlowTypeId == "Lateral Transport: BG Fast -> Aquatic" ~ "Q10 Slow Flows",
      FlowTypeId ==
        "Lateral Transport: BG Very Fast -> Aquatic" ~ "Q10 Slow Flows"
    )
  )
...
write.csv(
  myDataAll,
  paste0(outpathDatasheets, "stsim_FlowTypeGroupMembership2.csv"),
  row.names = FALSE
)

# Add to model
myCSV2 <- read.csv(paste0(
  outpathDatasheets,
  "stsim_FlowTypeGroupMembership2.csv"
))
...
myStocksCO2 <- tibble(
  StockTypeId = rep(c("Atmosphere: CO2", "Atmosphere"), each = 2),
  StockGroupId = rep(
    c(
      "Cumulative Emissions: CO2 (tons C)",
      "Cumulative Emissions: CO2 (tons CO2-eq)"
...
myStocksAll <- tibble(
  StockTypeId = rep(
    c("Atmosphere", "Atmosphere: CH4", "Atmosphere: CO", "Atmosphere: CO2"),
    each = 2
  ),
  StockGroupId = rep(
...
write.csv(
  myDataAll,
  paste0(outpathDatasheets, "stsim_StockTypeGroupMembership.csv"),
  row.names = FALSE
)
...
myCSV <- read.csv(paste0(
  outpathDatasheets,
  "stsim_StockTypeGroupMembership.csv"
))

myData <- datasheet(
  myScenario,
  name = "stsim_StockTypeGroupMembership",
  empty = TRUE,
  optional = TRUE
) %>%
  addRow(myCSV)
...
saveDatasheet(
  myScenario,
  myData,
  "stsim_StockTypeGroupMembership",
  append = TRUE
)
