...
myData <- data.frame(
  Timestep = seq(2000, 2022),
  FlowGroupId = "Net Growth Forest: Total",
  MultiplierFileName = paste0(
    inputRasterDir,
    "Growth Forest/forestNppMult_",
    seq(2000, 2022),
    ".tif"
  )
)
...
myData <- data.frame(
  ...
  MultiplierFileName = paste0(
    inputRasterDir,
    "Growth Non Forest/nonForestNppMult_",
    seq(2000, 2022),
    ".tif"
  )
) %>%
...
cmpModelInputsDir <- paste0(
  modelFullPath,
  "/Data/Transition Spatial Multipliers"
)
...
scenarios <- c("2001", "2006", "2010", "2016")

pathIn <- paste0(modelFullPath, "/Data/Initial Conditions/LUCAS/")

for (i in 2:length(scenarios)) {
  pattern1 <- paste0("LUCAS_", scenarios[i], ".tif")
  ...
  if (i == 2) {
    ...
    r2 <- ifel((r1 %in% c(95, 96) & r2 == 11), 13, r2)

    r2 <- ifel((r1 %in% c(90) & r2 == 11), 14, r2)

    r2 <- ifel((r1 %in% c(95, 96) & r2 == 97), 98, r2)

    r2 <- ifel((r1 %in% c(90) & r2 == 97), 99, r2)
...
for (i in 1:length(scenarios)) {
  file1 <- list.files(
    ...
    pattern = paste0("LUCAS_", scenarios[i], ".tif"),
    full.names = T
  )
...
lucas2001 <- rast(paste0(
  modelFullPath,
  "/Data/Initial Conditions/LUCAS/LUCAS_2001.tif"
))
lucas2006 <- rast(paste0(
  modelFullPath,
  "/Data/Initial Conditions/LUCAS/LUCAS_2006.tif"
))
lucas2010 <- rast(paste0(
  modelFullPath,
  "/Data/Initial Conditions/LUCAS/LUCAS_2010.tif"
))
lucas2016 <- rast(paste0(
  modelFullPath,
  "/Data/Initial Conditions/LUCAS/LUCAS_2016.tif"
))


stateClassIds <- unique(c(
  freq(lucas2001)$value,
...
lulcTransitions <- expand.grid(stateClassIds, stateClassIds) %>%
  ...
  rename(FromStateClass = Var1, ToStateClass = Var2)
...
for (i in 1:length(startYears)) {
  ...
  startYearRaster <- rast(paste0(
    modelFullPath,
    "/Data/Initial Conditions/LUCAS/LUCAS_",
    startYears[i],
    ".tif"
  ))
  endYearRaster <- rast(paste0(
    modelFullPath,
    "/Data/Initial Conditions/LUCAS/LUCAS_",
    endYears[i],
    ".tif"
  ...
  for (j in 1:nrow(lulcTransitions)) {
    ...
    transitionMultiplierRaster <- ifel(
      (startYearRaster == rowT$FromStateClass &
        endYearRaster == rowT$ToStateClass),
      1,
      0
    )

    lulcTransitionsReview$TotalPixelTransitionsObserved[j] <- sum(c(
      lulcTransitionsReview$TotalPixelTransitionsObserved[j],
      global(transitionMultiplierRaster, fun = "sum", na.rm = T)$sum
    ))

    outputFilename <- str_c(
      "tm-historical-",
      startYears[i],
      "-to-",
      endYears[i],
    ...
    transitionMultiplierRaster %>%
      writeRaster(
        filename = file.path(cmpModelInputsDir, "/", outputFilename),
        datatype = "INT2S",
        NAflag = -9999,
        overwrite = T
      )
...
transitionsKeep <- lulcTransitionsReview[
  lulcTransitionsReview$TotalPixelTransitionsObserved > 0,
]
...
tmFilepaths <- list.files(
  file.path(cmpModelInputsDir),
  pattern = ".tif",
  full.names = TRUE
)

fileNamesTable <- data.frame(
  Timestep = NA,
...
transitionTypesSub0 <- transitionTypesSub0[
  !(transitionTypesSub0 %in% myData$Name)
]

maxId <- max(myData$Id)

myDataNew <- data.frame(
  Name = transitionTypesSub0,
...
myData <- datasheet(myScenario, sheetName) %>%
  addRow(data.frame(
    StateClassIdSource = stateClassTable$Name,
...
transitionTypesSub <- gsub(
  " [Type]",
  "",
  gsub("LULCC: ", "", transitionTypes),
  fixed = T
)
...
myData <- datasheet(myScenario, sheetName, optional = T) %>%
  addRow(data.frame(
    StateClassIdSource = stateClassStart,
...
dependency(myScenario) <- c(
  "SF Flow Multipliers [Forested Wetland to Water]",
...
dependency(myScenario) <- c(
  "SF Flow Multipliers [Forested Wetland to Water IPCC]",
  "SF Flow Multipliers [Emergent Wetland to Water IPCC]",
  "SF Flow Multipliers [Non Forest; Updated]",
  "SF Flow Multipliers [Emergent Wetland, Site]",
  "SF Flow Multipliers [Forested Wetland BGS Slower; Add Lat; Site]",
...
myData <- datasheet(myScenario, name = sheetName, empty = TRUE) %>%
  addRow(data.frame(RasterOutputSC = TRUE, RasterOutputSCTimesteps = 1))
saveDatasheet(myScenario, myData, sheetName, append = FALSE)
...
myData <- datasheet(myScenario, name = sheetName, empty = TRUE) %>%
  addRow(data.frame(RasterOutputSC = TRUE, RasterOutputSCTimesteps = 15))
saveDatasheet(myScenario, myData, sheetName, append = FALSE)
...
if (turnOnSpatialMultiprocessing) {
  ...
  spatialMultiProcessingPath <- paste0(
    modelFullPath,
    "/Data/Spatial Multi Processing"
  )
  ...
  initialScenario <- scenario(myProject, "STSM Initial Conditions [Spatial]")

  classRaster <- rast(paste0(
    modelFullPath,
    "/",
    modelName,
    ".ssim.data/Scenario-",
  ...
  # Calculate ncol and nrow for one tile - fine enough to balance non-NA
  # cells across numberOfTiles regardless of how much of the extent is NA
  tileDimension <- ceiling(sqrt(numberOfTiles * oversampleFactor))

  # Generate smp grid
  smallGrid <- rast(
    ncols = tileDimension,
    nrows = tileDimension,
    vals = seq(tileDimension^2)
  )
  ...
  # Split counts (in spatial/adjacency order) into exactly nTiles contiguous
  # groups, closing each group once its cumulative share reaches the ideal
  # per-tile target - guarantees exactly nTiles groups when there are at
  # least nTiles non-empty initial partitions.
  partitionContiguous <- function(input, nTiles) {
    target <- sum(input) / nTiles
    cumulator <- 0
    groupIndex <- 1
    ...
    for (i in seq_along(input)) {
      ...
      output[i] <- groupIndex
      while (groupIndex < nTiles && cumulator >= groupIndex * target) {
  ...
  # Assign counts to exactly nTiles bins with no contiguity requirement, via
  # longest-processing-time bin packing: largest items first, each going to
  # the currently smallest bin. Guarantees exactly nTiles groups.
  partitionBalanced <- function(input, nTiles) {
    binSums <- numeric(nTiles)
    output <- integer(length(input))
    for (i in order(input, decreasing = TRUE)) {
      b <- which.min(binSums)
      output[i] <- b
      binSums[b] <- binSums[b] + input[i]
    }
    return(output)
  }

  freqTable <- freq(maskedGrid) %>% as_tibble

  if (nrow(freqTable) < numberOfTiles) {
    stop(
      "Only ",
      nrow(freqTable),
      " non-empty initial partitions for ",
      numberOfTiles,
      " requested tiles - increase oversampleFactor."
    )
  }
  ...
  rcl <- freqTable %>%
    ...
    mutate(
      newValue = if (!contig) {
        partitionBalanced(count, numberOfTiles)
      } else {
        partitionContiguous(count, numberOfTiles)
      }
    ) %>%
...
folder(ssimObject = myProject, folder = "4. Final Spatial Scenarios")
