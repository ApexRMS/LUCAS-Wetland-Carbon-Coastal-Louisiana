...
filePaths <- c(paste0(modelFullPath, "/Data"),
...
for (i in 1:length(filePaths)){
  if(!dir.exists(filePaths[i])){dir.create(filePaths[i])}
}
...
if (applyCoarserGrid == TRUE){
  ...
  studyAreaMask <- aggregate(primaryStrataRaster,
                             fact = spatialResMult,
                             fun = "modal",
                             na.rm=TRUE)
...
## Write study area rasters to disk
writeRaster(studyAreaMask, paste0(modelFullPath, "/Data/Initial Conditions/PrimaryStrata.tif"), overwrite=T)
...
png(file=paste0(modelFullPath, "/Images", "/CCAP.png"), width=800, height=800)
plot(ccapStack)
dev.off()

# Write NLCD data to disk
writeRaster(ccapStack, paste0(modelFullPath, "/Data/Initial Conditions/CCAP/", names(ccapStack), ".tif"), overwrite=T)
...
if (applyCoarserGrid == TRUE){
  ...
  forestTypeGroup <- aggregate(forestTypeGroup,
                        fact = spatialResMult,
                        fun = "modal",
                        na.rm=TRUE)
...
forestTypeGroupNAs <- ifel((is.na(forestTypeGroupFilled) &
                              forestStack == 1),NA,1)
...
reclassTable <- data.frame(from = c(0:25),
                           to = c(NA,NA,24,23,22,21,
                                  82,81,
                                  71,
                                  600,600,600,
                                  54,
...
png(file=paste0(modelFullPath, "/Images", "/LUCAS.png"), width=800, height=800)
plot(lucasStack)
dev.off()

# Write NLCD data to disk
writeRaster(lucasStack, paste0(modelFullPath, "/Data/Initial Conditions/LUCAS/", names(lucasStack), ".tif"), overwrite=T)
...
standAgeNAs <- ifel((is.na(standAgeFilled) &
                       forestWetlandStack == 1),NA,1)
...
if(all(naCount$count == ncell(standAgeFilled),minmax(forestWetlandStack)[2] == 1)){
  standAgeFilled <- standAgeFilled %>% setValues(values = 63)
} else if(all(naCount$count == ncell(standAgeFilled),minmax(forestWetlandStack)[2] == 0)){
...
standAgeRaster <- ifel(lucas2001 >= 100 | lucas2001==90 | lucas2001==91, standAgeFilled, 0)
...
terra::writeRaster(lucas2001, paste0(modelFullPath, "/Data/Initial Conditions/", "State Class", ".tif"), overwrite=T)
...
forestNppNormal <- ifel(fMapForestNormal < fMatForestNormal, fMapForestNormal, fMatForestNormal)
...
# Forest
forestNppMult <- terra::project(forestNppAnomoly, studyAreaMask, method = "near")
forestNppMult <- mask(forestNppMult, studyAreaMask)
names(forestNppMult) <- paste0("forestNppMult_", seq(2000,2022))

# Non Forest
nonforestNppMult <- terra::project(nonForestAnomoly, studyAreaMask, method = "near")
